...
def main():
    ...
    for i in range(len(ds)):
        ...
        raw_key = norm_name(raw_name)

        # Oxford-IIIT Pet calls this breed "german_shorthaired",
        # while our ontology uses the canonical project name
        # "german_shorthaired_pointer".
        key = {
            "german_shorthaired": "german_shorthaired_pointer"
        }.get(raw_key, raw_key)
    ...
    with torch.no_grad():
        for j in range(len(subset)):
            ...
            key = {"german_shorthaired": "german_shorthaired_pointer"}.get(key, key)
            inputs = processor(images=image, return_tensors="pt")
            inputs = {k: v.to(device) for k, v in inputs.items()}
            emb = clip(**inputs).image_embeds[0].float().cpu()
            embeddings.append(emb)
    ...
    loss_log_path = os.path.join(args.out, "training_loss_log.json")
